...
//Ajouter Produit
function AjouterProduit($libelle,$prix,$quantite,$image,$categorie,$type,$description, $db)
{
   $sql = "INSERT INTO `produits`(`libelle`, `prix`, `quantite`, `image`,`categorie`,`type`, `description`) VALUES (:libelle,:prix,:quantite,:image,:categorie,:type,:description)";
   $stat= $db->prepare($sql);
   $data=[
        'libelle' => $libelle,
        'prix' => $prix,
...
//Modifier Produit
function ModifierProduit($idp,$libelle,$prix,$quantite,$categorie,$type,$description, $db)
{
   $sql = "UPDATE `produits` SET `libelle`=:libelle,`prix`=:prix,`quantite`=:quantite,`categorie`=:categorie,`type`=:type,`description`=:description WHERE id=:idp";
   $stat= $db->prepare($sql);
   $data=[
        'libelle' => $libelle,
        'prix' => $prix,
