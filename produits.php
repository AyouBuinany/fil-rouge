<?php
	...
	if(isset($_POST["Recherche"])){
		...
		if($categorie=="all"){
						...
						$query3 = $db->prepare("SELECT * FROM produits where quantite>0 AND type='$type'");
						}
		}else{
						if($type=="all"){
						$query3 = $db->prepare("SELECT * FROM produits where quantite>0 AND categorie='$categorie'");
						}else{
						$query3 = $db->prepare("SELECT * FROM produits where quantite>0 AND categorie='$categorie' AND type='$type'");
						}
	}
	$query3->execute();
...
<select class="custom-select" id="inputGroupSelect04" aria-label="Example select with button addon" name="categorie">
	...
	<option value="all"> neuf/utilisé</option>
				...
				<div class="col-md-8 col-lg-12 order-md-last">
							...
							<div class="product">
								...
								<div class="text py-3 px-3">
									...
									<div class="d-flex">
										...
										<div class="rating">
											<p class="text-right type"> <?php echo $produit['type'] ?></span></p>
										</div>
									</div>
					...
					<div class="row mt-5">
						<div class="col text-center">
							<div class="block-27">
							Tous les vêtements pour homme printemps-été 2020 LuxForAll. Découvre tous les vêtements de la nouvelle collection !
							</div>
						</div>
					</div>
				</div>
